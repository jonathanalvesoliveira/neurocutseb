...
class Rule:
    ...
    def __str__(self):
        ...
        for i in range(len(self.names)):
            if(self.names[i] == 'src_ip' or self.names[i] == 'dst_ip'):
                try:
                    result += "%s:[%s, %s) " % (self.names[i], str(ipaddress.IPv4Address(self.ranges[i * 2])),
                                        str(ipaddress.IPv4Address(self.ranges[i * 2 + 1])))
